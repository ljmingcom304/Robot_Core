...
/**
 * Title:TextViewLayout
 * <p>
 * Description:子控件自适应并自动换行
 * </p>
 * Author Jming.L
 * Date 2017/9/22 15:49
 */
public class TextViewLayout extends LinearLayout {// 此处若继承ViewGroup会产生意想不到的错误
	...
	private boolean mAverage = true;

	public TextViewLayout(Context context) {
		this(context, null);
	}

	public TextViewLayout(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		this.context = context;
		DEFAULT_SPACING = dip2px(10);
	}
	...
	private void requestLayoutInner() {
		Runnable runnable = new Runnable() {
			@Override
			public void run() {
				requestLayout();// 重新测量与布局
			}
		};
		if (Looper.myLooper() != Looper.getMainLooper()) {// 判断当前线程是否在主线程中
			runnable.run();
		} else {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		if (getChildCount() == 0) {// 当不包含子控件时，直接返回
		...
		// Log.e("布局", totalWidth+":"+sizeWidth);
		// setMeasuredDimension(totalWidth,totalHeight);//将数据写死解决布局混乱的问题
		setMeasuredDimension(totalWidth, resolveSize(totalHeight, heightMeasureSpec));
	}

	@Override
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		if (getChildCount() == 0) {// 当不包含子控件时，直接进行返回
			return;
		}
	...
	class Line {
		...
		public void layoutView(int l, int t) {// 布局
			...
			int surplusWidth = mAverage?layoutWidth - mWidth - mHorizontalSpacing * (count - 1):0;
