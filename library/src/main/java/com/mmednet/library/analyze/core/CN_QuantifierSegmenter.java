 ...
 * 
 */
package com.mmednet.library.analyze.core;
...
import com.mmednet.library.analyze.dic.Hit;
...
class CN_QuantifierSegmenter implements ISegmenter{
	 ...
	 * 分词
	 */
	public void analyze(com.mmednet.library.analyze.core.AnalyzeContext context) {
		//处理中文数词
	 ...
	 * 处理数词
	 */
	private void processCNumber(com.mmednet.library.analyze.core.AnalyzeContext context){
	 ...
	 * @param context
	 */
	private void processCount(com.mmednet.library.analyze.core.AnalyzeContext context){
		// 判断是否需要启动量词扫描
		...
		if(CharacterUtil.CHAR_CHINESE == context.getCurrentCharType()){
			...
			if(!this.countHits.isEmpty()){
				...
				for(Hit hit : tmpArray){
					...
					if(hit.isMatch()){
						//输出当前的词
						com.mmednet.library.analyze.core.Lexeme newLexeme = new com.mmednet.library.analyze.core.Lexeme(context.getBufferOffset() , hit.getBegin() , context.getCursor() - hit.getBegin() + 1 , com.mmednet.library.analyze.core.Lexeme.TYPE_COUNT);
			...
			if(singleCharHit.isMatch()){//首字成量词词
				//输出当前的词
				com.mmednet.library.analyze.core.Lexeme newLexeme = new com.mmednet.library.analyze.core.Lexeme(context.getBufferOffset() , context.getCursor() , 1 , com.mmednet.library.analyze.core.Lexeme.TYPE_COUNT);
				context.addLexeme(newLexeme);
	...
	private boolean needCountScan(AnalyzeContext context){
		if((nStart != -1 && nEnd != -1 ) || !countHits.isEmpty()){
			...
			if(!context.getOrgLexemes().isEmpty()){
				...
				if(com.mmednet.library.analyze.core.Lexeme.TYPE_CNUM == l.getLexemeType() ||  com.mmednet.library.analyze.core.Lexeme.TYPE_ARABIC == l.getLexemeType()){
	...
	private void outputNumLexeme(AnalyzeContext context){
		if(nStart > -1 && nEnd > -1){
			//输出数词
			Lexeme newLexeme = new Lexeme(context.getBufferOffset() , nStart , nEnd - nStart + 1 , com.mmednet.library.analyze.core.Lexeme.TYPE_CNUM);
			context.addLexeme(newLexeme);
