...
public final class MonthViewPager extends ViewPager {
    ...
    private void updateMonthViewHeight(int year, int month) {
        ...
        if (mParentLayout != null) {
            if (getVisibility() != VISIBLE) {//如果已经显示周视图，则需要动态改变月视图高度
                ViewGroup.LayoutParams params = getLayoutParams();
                params.height = DateUtil.getMonthViewHeight(year, month, mDelegate.getCalendarItemHeight());
                setLayoutParams(params);
            }
            mParentLayout.updateContentViewTranslateY();
        }
        mCurrentViewHeight = DateUtil.getMonthViewHeight(year, month, mDelegate.getCalendarItemHeight());
        if (month == 1) {
            mPreViewHeight = DateUtil.getMonthViewHeight(year - 1, 12, mDelegate.getCalendarItemHeight());
            mNextViewHeight = DateUtil.getMonthViewHeight(year, 2, mDelegate.getCalendarItemHeight());
        } else {
            mPreViewHeight = DateUtil.getMonthViewHeight(year, month - 1, mDelegate.getCalendarItemHeight());
            if (month == 12) {
                mNextViewHeight = DateUtil.getMonthViewHeight(year + 1, 1, mDelegate.getCalendarItemHeight());
            } else {
                mNextViewHeight = DateUtil.getMonthViewHeight(year, month + 1, mDelegate.getCalendarItemHeight());
            }
        }
    }
    ...
    void scrollToCalendar(int year, int month, int day, boolean smoothScroll) {
        ...
        if (mParentLayout != null) {
            int i = DateUtil.getWeekFromDayInMonth(calendar);
