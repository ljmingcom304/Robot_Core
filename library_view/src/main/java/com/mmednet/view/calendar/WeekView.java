...
public abstract class WeekView extends BaseView {
    ...
    public void onClick(View v) {
        if (isClick) {
            ...
            if (calendar != null) {
                if (!DateUtil.isCalendarInRange(calendar, mDelegate.getMinYear(),
    ...
    void performClickCalendar(Calendar calendar, boolean isNotice) {
        ...
        if (mItems.contains(mDelegate.getCurrentDay())) {
            week = DateUtil.getWeekFormCalendar(mDelegate.getCurrentDay());
        }
        ...
        if (!DateUtil.isCalendarInRange(currentCalendar, mDelegate.getMinYear(),
                mDelegate.getMinYearMonth(), mDelegate.getMaxYear(), mDelegate.getMaxYearMonth())) {
        ...
        int i = DateUtil.getWeekFromDayInMonth(currentCalendar);
        mParentLayout.setSelectWeek(i);
    ...
    private int getEdgeIndex(boolean isMinEdge) {
        for (int i = 0; i < mItems.size(); i++) {
            ...
            if (isMinEdge && DateUtil.isCalendarInRange(item, mDelegate.getMinYear(), mDelegate.getMinYearMonth(),
                    mDelegate.getMaxYear(), mDelegate.getMaxYearMonth())) {
                return i;
            } else if (!isMinEdge && !DateUtil.isCalendarInRange(item, mDelegate.getMinYear(), mDelegate.getMinYearMonth(),
                    mDelegate.getMaxYear(), mDelegate.getMaxYearMonth())) {
                return i - 1;
            }
        }
    ...
    void setup(Calendar calendar) {
        ...
        int dayCount = DateUtil.getMonthDaysCount(calendar.getYear(), calendar.getMonth());//获取某个月有多少天
        ...
        if (calendar.getDay() - week <= 0) {//如果某月某天-星期<0，则说明前面需要上个月的补数
            ...
            if (calendar.getMonth() == 1) {//取上一年的12月份
                ...
                preMonthDaysCount = DateUtil.getMonthDaysCount(calendar.getYear(), calendar.getMonth() - 1);
