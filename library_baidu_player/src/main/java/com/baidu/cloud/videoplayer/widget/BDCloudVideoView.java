...
public class BDCloudVideoView extends FrameLayout implements MediaController.MediaPlayerControl {
    ...
    private static boolean mProxyCacheEnable = false;
    private int switchMode = BDCloudMediaPlayer.SOURCE_SWITCH_NORMAL_MODE;
    private String[] mMediaItems;
    private String[] mMutilMediasDsc;
    private boolean mShowSourceSwitchInfo = true;
    ...
    private void setCurrentState(PlayerState newState, boolean force) {
        if (force || mCurrentState != newState) {
            mCurrentState = newState;
            if (mOnPlayerStateListener != null) {
                mOnPlayerStateListener.onPlayerStateChanged(mCurrentState);
            }
    ...
    private int mMaxCacheDurationInMs = -1;
    ...
    private Handler mainThreadHandler = new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            } else if (msg.what == IMediaPlayer.MEDIA_INFO_MEDIA_CHANGE_START) {
                Toast.makeText(mAppContext, "清晰度切换中...", Toast.LENGTH_SHORT).show();
            } else if (msg.what == IMediaPlayer.MEDIA_INFO_MEDIA_CHANGE_END) {
                int success = msg.arg2;
                if (success < 0) {
                    Toast.makeText(mAppContext, "清晰度切换失败!", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(mAppContext, "清晰度切换成功！", Toast.LENGTH_SHORT).show();
                }
            }
//            super.handleMessage(msg);
    ...
    private void showSourceSwitchState(int what, int extra) {
        if (mShowSourceSwitchInfo) {
            Message msg = mainThreadHandler.obtainMessage();
            msg.what = what;
            msg.arg1 = extra;
            mainThreadHandler.sendMessage(msg);
    ...
    public void setVideoPathWithToken(String path, String token) {
        ...
        if (mProxyCacheEnable  && switchMode == BDCloudMediaPlayer.SOURCE_SWITCH_NORMAL_MODE) {
            String proxyPath = mProxyCacheManager.getProxyUrl(mAppContext, path);
            mProxyCacheManager.setCacheAvailableListener(new CacheListener() {
                @Override
                public void onCacheAvailable(File cacheFile, String url, int percentsAvailable) {
                    Log.d(TAG,"percents:" + percentsAvailable +" file" + cacheFile + " url:" + url);
                }
            });
            path = proxyPath;
        }
        Log.w(TAG, "Video Uri: " + path);
...
//            mTargetState = STATE_ERROR;
    ...
    public BDCloudMediaPlayer createPlayer() {
        ...
        if (mMediaItems != null) {
            bdCloudMediaPlayer.setMediaItems(mMediaItems);
        }
        if (mMaxCacheDurationInMs >= 0) {
            bdCloudMediaPlayer.setMaxCacheDuration(mMaxCacheDurationInMs);
        }

        bdCloudMediaPlayer.setMediaInputType(switchMode);
    ...
    /**
     * Enable or disable video cache
     * @param isEnable If enable cache
     * @param cachePath Cache Path. use default dir if cachePath is invalid
     */
    public void setVideoCacheEnabled(boolean isEnable, String cachePath) {
        mProxyCacheEnable = isEnable;
        if (!TextUtils.isEmpty(cachePath)) {
            mProxyCacheManager.setCacheDir(new File(cachePath));
        }
    }
    ...
    IMediaPlayer.OnVideoSizeChangedListener mSizeChangedListener =
            new IMediaPlayer.OnVideoSizeChangedListener() {
                public void onVideoSizeChanged(IMediaPlayer mp, int width, int height, int sarNum, int sarDen) {
                    ...
                    setCurrentState(PlayerState.STATE_VIDEOSIZE_CHANGED, true);
                }
            };
                ...
                public boolean onInfo(IMediaPlayer mp, int arg1, int arg2) {
                    ...
                    switch (arg1) {
                        ...
                        case IMediaPlayer.MEDIA_INFO_AUDIO_RENDERING_START:
                            ...
                            break;
                        case IMediaPlayer.MEDIA_INFO_MEDIA_CHANGE_START:
                            Log.i(TAG, " MEDIA_INFO_MEDIA_CHANGE_START:");
                            showSourceSwitchState(arg1, 0);
                            break;
                        case IMediaPlayer.MEDIA_INFO_MEDIA_CHANGE_END:
                            showSourceSwitchState(arg1, arg2);
                            Log.d(TAG, " MEDIA_INFO_MEDIA_CHANGE_END:");
                            break;
                    }
                    return true;
                }
...
//                    Log.d(TAG, "onBufferingUpdate: percent=" + percent);
    ...
    public void release() {
        ...
        if (mProxyCacheEnable && mProxyCacheManager != null){
    ...
    public void start() {
        if  (mMediaPlayer != null) {
            if ((mCurrentState == PlayerState.STATE_ERROR)
    ...
    public  void setMediaItems(String[] mediaItems) {
    ...
    /**
     * 设置多分辨率媒体流信息
     * @param mediaItems 多分辨率媒体流
     * @param mutilMediasDsc 格式：VideoWidthxVideoWidth,Bitrate(1280x720,650000)
     */
    public  void setMediaItems(String[] mediaItems, String[] mutilMediasDsc) {
        mMutilMediasDsc = mutilMediasDsc;
        setMediaItems(mediaItems);
    }

    public  String[] getMutilMediasDsc() {
        return mMutilMediasDsc;
    }
    ...
    public String[] getVariantInfo() {
        if (mMediaPlayer != null) {
            if (switchMode ==  BDCloudMediaPlayer.SOURCE_SWITCH_SMOOTH_MODE
                    || switchMode == BDCloudMediaPlayer.SOURCE_SWITCH_SMOOTH_HLS_MODE)  {
                return mMediaPlayer.getMediaItems();
            } else {
    ...
    public void setMediaInputType(int mode) {
        switchMode = mode;
        if (mode != BDCloudMediaPlayer.SOURCE_SWITCH_NORMAL_MODE) {
            // 加速切换效率，可以将缓冲设置小，建议为视频的2个gop长度
            mMaxCacheDurationInMs = 10 * 1000;
        }
        if (mMediaPlayer != null) {
            mMediaPlayer.setMediaInputType(mode);
            if (mode != BDCloudMediaPlayer.SOURCE_SWITCH_NORMAL_MODE) {
                mMediaPlayer.setMaxCacheDuration(mMaxCacheDurationInMs);
            }
        }
    }
    ...
    public static void setCuid(String cuid) {
        BDCloudMediaPlayer.setCuid(cuid);
    }
